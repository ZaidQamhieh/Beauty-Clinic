...
class DoctorDashboardData {
  ...
  factory DoctorDashboardData.fromJson(
    Map<String, dynamic> json, {
    AdminDateRangeType rangeType = AdminDateRangeType.days30,
    int? intervalCount,
  }) {
    final rawAppointmentsOverTime =
        (json['appointmentsOverTime'] as List<dynamic>?)
            ?.map(
              (e) => PatientGrowthPoint(
            ...
            .toList() ??
        [];
    final appointmentsOverTimeList = _groupIntoIntervals(
      rawAppointmentsOverTime,
      intervalCount ?? _defaultIntervalCount(rangeType),
    );

    final treatmentsList =
        (json['treatmentsPerformed'] as List<dynamic>?)
    ...
    final ServiceAnalyticsData serviceAnalytics = ServiceAnalyticsData(
      ...
      topService: treatmentsList.isNotEmpty
          ? treatmentsList.first.serviceName
          : 'Consultation',
    ...
    final todayAppts =
        (json['todayAppointments'] as List<dynamic>?)
            ?.map(
              (e) => DoctorAppointmentItem.fromJson(e as Map<String, dynamic>),
            )
            .toList() ??
        [];
    ...
    return DoctorDashboardData(
      ...
      appointmentOutcomes: json['appointmentOutcomes'] != null
          ? AppointmentOutcomesData.fromJson(
              json['appointmentOutcomes'] as Map<String, dynamic>,
            )
  ...
  static int _defaultIntervalCount(AdminDateRangeType rangeType) {
    return switch (rangeType) {
      AdminDateRangeType.days7 => 7,
      AdminDateRangeType.days30 => 4,
      AdminDateRangeType.months3 => 12,
      AdminDateRangeType.custom => 4,
    };
  }

  static List<PatientGrowthPoint> _groupIntoIntervals(
    List<PatientGrowthPoint> dailyPoints,
    int intervalCount,
  ) {
    if (dailyPoints.isEmpty) return const [];
    final count = intervalCount.clamp(1, dailyPoints.length);
    final points = List<PatientGrowthPoint>.filled(
      count,
    ...
    for (var index = 0; index < dailyPoints.length; index++) {
      final bucket = (index * count ~/ dailyPoints.length).clamp(0, count - 1);
      final current = points[bucket];
      points[bucket] = PatientGrowthPoint(
        dateLabel: count <= 7
            ? dailyPoints[index].dateLabel
            : 'Week ${bucket + 1}',
        totalCumulative:
            current.totalCumulative + dailyPoints[index].totalCumulative,
        monthlyNew: current.monthlyNew + dailyPoints[index].monthlyNew,
      );
    }
    return points;
  }
...
class DoctorDashboardRepository {
  static Future<DoctorDashboardData> fetchDashboardData(
    ApiClient? apiClient, {
    AdminDateRangeType rangeType = AdminDateRangeType.days30,
    ...
    if (apiClient != null) {
      try {
        ...
        final start = switch (rangeType) {
          ...
          AdminDateRangeType.custom =>
            customRange?.start ?? now.subtract(const Duration(days: 30)),
        };
        final end = rangeType == AdminDateRangeType.custom
            ? customRange?.end ?? now
            : now;
        ...
        if (response.data is Map<String, dynamic>) {
          return DoctorDashboardData.fromJson(
            response.data as Map<String, dynamic>,
            rangeType: rangeType,
            intervalCount: _intervalCount(rangeType, customRange),
          );
        }
      } catch (_) {
    ...
    return _offlineMock(rangeType);
  }

  static int _intervalCount(
    AdminDateRangeType rangeType,
    DateTimeRange? customRange,
  ) {
    if (rangeType != AdminDateRangeType.custom || customRange == null) {
      return DoctorDashboardData._defaultIntervalCount(rangeType);
    }
    final days = customRange.duration.inDays + 1;
    if (days <= 7) return days;
    if (days <= 31) return 4;
    if (days <= 100) return 12;
    return (days / 30).ceil().clamp(1, 12);
  }

  static DoctorDashboardData _offlineMock(AdminDateRangeType rangeType) {
    return DoctorDashboardData(
