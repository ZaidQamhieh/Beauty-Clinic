...
void main() {
  group('Admin Analytics Repository & Models', () {
    test('fetchDashboardData returns valid structure for 7 Days, 30 Days, 3 Months', () {
      final d7 = AdminAnalyticsRepository.fetchDashboardData(rangeType: AdminDateRangeType.days7);
      ...
      final d30 = AdminAnalyticsRepository.fetchDashboardData(rangeType: AdminDateRangeType.days30);
      expect(d30.rangeType, AdminDateRangeType.days30);
      expect(d30.overview.totalPatients, greaterThan(0));

      final d3m = AdminAnalyticsRepository.fetchDashboardData(rangeType: AdminDateRangeType.months3);
      expect(d3m.rangeType, AdminDateRangeType.months3);
      expect(d3m.overview.totalPatients, greaterThan(0));
    });

    test('AdminDashboardData.fromJson correctly parses full backend response payload', () {
      final json = {
      ...
      final data = AdminDashboardData.fromJson(json, rangeType: AdminDateRangeType.days30);
      expect(data.overview.totalPatients, 1284);
      expect(data.serviceAnalytics.topService, 'HydraFacial Glow');
      expect(data.doctorAnalytics.utilizationList.first.doctorName, 'Dr. Hana Nasser');
      expect(data.appointmentAnalytics.outcomes.completed, 78);
      expect(data.patientAnalytics.retention.retentionRate, 86.4);
      expect(data.operations.todayAppointments.first.patientName, 'Nour Al-Khalil');
      expect(data.operations.staffList.first.name, 'Dr. Hana Nasser');
    });

    test('fetchDashboardDataAsync falls back gracefully when apiClient is null', () async {
  ...
  group('Admin Analytics Widgets', () {
    testWidgets('AdminDateFilterBar displays range chips and reacts to selection', (tester) async {
    ...
    testWidgets('ServiceBookingsBarChart renders rows correctly', (tester) async {
      final data = AdminAnalyticsRepository.fetchDashboardData().serviceAnalytics;
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
